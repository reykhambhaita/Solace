...
export function testLanguageDetection() {
  const tests = [
    // Small snippets
    {
      name: 'TypeScript interface (2 lines)',
      code: `interface User {
  name: string;
}`,
      expected: { language: 'typescript', minConfidence: 0.90 },
    },
    {
      name: 'Go package main (1 line)',
      code: `package main`,
      expected: { language: 'go', minConfidence: 0.95 },
    },
    {
      name: 'Rust fn main (1 line)',
      code: `fn main() {`,
      expected: { language: 'rust', minConfidence: 0.95 },
    },
    {
      name: 'Python __init__ (2 lines)',
      code: `def __init__(self, name):
    self.name = name`,
      expected: { language: 'python', minConfidence: 0.90 },
    },
    {
      name: 'PHP opening tag',
      code: `<?php
$name = "test";`,
      expected: { language: 'php', minConfidence: 0.98 },
    },

    // Medium snippets with negative patterns
    {
      name: 'TypeScript with no Python patterns',
      code: `interface User {
  name: string;
  age: number;
}
...
func main() {
    ...
    time.Sleep(1 * time.Second)
      ...
      expected: { language: 'go', minConfidence: 0.90 },
    },
    {
      name: 'Rust with borrowing',
      code: `fn main() {
...
fn change(s: &mut String) {
    s.push_str(", world");
      ...
      expected: { language: 'rust', minConfidence: 0.90 },
    },
    {
      name: 'Java with System.out',
...
public class HelloWorld {
    public static void main(String[] args) {
      ...
      expected: { language: 'java', minConfidence: 0.92 },
    },
    {
      name: 'C++ with std::',
...
int main() {
    ...
    return 0;
      ...
      expected: { language: 'cpp', minConfidence: 0.88 },
    },
    {
      name: 'C with malloc (not C++)',
...
int main() {
    ...
    return 0;
      ...
      expected: { language: 'c', minConfidence: 0.80 },
    },
    {
      name: 'Ruby with .each do',
      code: `class Person
  attr_accessor :name
...
[1, 2, 3].each do |num|
  puts num
      ...
      expected: { language: 'ruby', minConfidence: 0.85 },
    },
  ];

  console.log('Enhanced Language Detection Tests:\n');

  let passed = 0;
  let failed = 0;

  for (const test of tests) {
    const result = detectLanguage(test.code);
    const success =
    ...
    if (success) {
      ...
      console.log(`  Detected: ${result.language} (${result.dialect || 'N/A'})`);
      console.log(`  Confidence: ${result.confidence} (Method: ${result.detectionMethod})`);
      console.log(`  Indicators: ${result.indicators.slice(0, 3).join(', ')}\n`);
    } else {
      failed++;
      console.log(`✗ ${test.name}`);
      console.log(`  Expected: ${test.expected.language} (≥${test.expected.minConfidence})`);
      console.log(`  Got: ${result.language} (${result.confidence}, ${result.detectionMethod})`);
